...
위·아래 이동·확인·취소는 Pi의 `tui.select.up`/`down`/`confirm`/`cancel` 키 바인딩을 따르며 도움말에도 해석된 실제 설정값을 표시합니다. `matches()`만 있고 `getKeys()`가 없는 관리자는 `matches()`의 거부도 권위 있으므로 기본 키로 대체하지 않습니다. `getKeys()`가 명시적으로 빈 목록을 주거나 `getKeys()`/`matches()` 호출이 실패할 때만 기본 키를 검토합니다. 이때도 다른 설정된 선택 동작이 그 입력을 소유하면 대체 키는 비활성화되며 도움말에는 `Unbound`로 표시합니다. 예를 들어 `confirm=[]`, `cancel=[Enter]`이면 Enter는 취소만 하고 확인의 Enter 대체는 생기지 않습니다. 이는 모달 프롬프트의 완전한 미바인딩은 복구하되 사용자 설정 동작을 덮어쓰지 않기 위함입니다.
...
| fullscreen 마우스 | 옵션 레이블·설명 클릭, 질문/검토 탭 이동, 옵션 위 휠 한 행 이동 (`press`는 커서·focus만 이동) |

마우스 동작은 Pi fullscreen에만 적용됩니다. 다중 선택의 클릭은 해당 항목만 토글하고 확정하지 않으며, 자유 입력 중 다른 옵션·탭을 클릭하면 편집 버퍼를 취소합니다. 장식·빈 공간과 옵션 밖의 휠은 처리하지 않습니다.

자유 입력 편집기는 Pi의 `tui.input.submit` 바인딩으로 제출하므로 도움말도 그 키와 해석된 취소 키를 표시합니다. 선택 목록의 확인 키와는 별개입니다. `submit`을 비워도 `Enter`가 다른 편집기 동작에 할당되지 않았을 때만 `Enter`를 대체 제출 키로 사용합니다. 예를 들어 `tui.input.newLine`에 `Enter`를 할당하면 그 동작을 가로채지 않으며, 제출할 키가 없으므로 자유 입력 항목을 표시하지 않습니다.
...
- 취소된 UI 결과에는 `cancelReason`과 이미 답한 항목이 남습니다. 자유 입력은 앞뒤 공백을 제거해 저장합니다. 가능한 결과가 Pi의 50KB·2000줄 제한을 넘는 설문은 UI를 열지 않고 오류 결과로 반환하므로 어떤 답변 레코드도 텍스트에서 잘리지 않습니다.
